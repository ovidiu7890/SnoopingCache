...
architecture Behavioral of cache is
    ...
    signal r_is_wb       : std_logic := '0'; -- NEW: Remembers if we need to Write-Back
...
begin
    ...
    -- 2. [COMBINATORIAL] Snoop Hit Detection
    ...
    begin
        if i_rst = '1' then
            ...
            r_state          <= S_IDLE;
            o_mem_request    <= '0'; o_rwitm <= '0'; o_writeback_en <= '0';
            o_read_data_cpu  <= (others => '0'); o_invalidate <= '0';
            r_is_upgrade     <= '0';
            r_is_wb          <= '0'; -- Reset WB flag
            
        elsif rising_edge(i_clk) then
            
            -- Keep o_writeback_en high if the flag is set
            o_writeback_en <= r_is_wb; 

            -- A. APPLY SNOOP UPDATES
            if i_data_request = '1' then
                for i in 0 to 15 loop
                    if r_cache_lines(i)(47 downto 34) = i_address_cc(15 downto 2) then
                        if r_cache_lines(i)(33 downto 32) /= "11" then
            ...
            case r_state is
                when S_IDLE =>
                    if (i_write = '1') or (i_read = '1') then
                        if s_hit = '1' then
                            ...
                            -- HIT WRITE
                            if i_write = '1' then
                                if r_cache_lines(0)(33 downto 32) = "10" then -- Shared -> Upgrade
                                    o_mem_request <= '1';
                                    ...
                                    r_cache_lines(0)(33 downto 32) <= "00"; -- Modified
                            ...
                            if r_cache_lines(15)(33 downto 32) = "00" then
                                r_is_wb        <= '1'; -- SET WB FLAG
                                o_writeback_en <= '1';
                                o_data_out     <= r_cache_lines(15);
                            end if;

                            if i_write = '1' then
                                o_rwitm <= '1'; r_state <= S_MISS_WAIT_WRITE;
                            elsif i_read = '1' then
                                o_rwitm <= '0'; r_state <= S_MISS_WAIT_READ;
                            end if;
                        end if;
                    end if;

                when S_MISS_WAIT_WRITE =>
                    o_mem_request <= '1'; o_rwitm <= '1'; -- Hold Request
                    
                    if i_mem_ready = '1' then
                        o_mem_request  <= '0'; 
                        o_rwitm        <= '0';
                        r_is_wb        <= '0'; -- Clear WB Flag
                        o_writeback_en <= '0';
                ...
                when S_MISS_WAIT_READ =>
                    o_mem_request <= '1'; o_rwitm <= '0'; -- Hold Request
