...
begin
    ...
    -- Write on ABORT (Flush) or WB
    s_ram_write <= '1' when (s_bus_active='1' and s_bus_abort='1') else
                   '1' when (s_bus_active='1' and s_cache_wb_en(s_grant_id)='1') else '0';
    
    -- Read only if NOT Aborting
    s_ram_read <= '1' when (s_bus_active='1' and (s_sys_cmd=CMD_BUS_RD or s_sys_cmd=CMD_BUS_RDX) 
                            and s_bus_abort='0' and s_cache_wb_en(s_grant_id)='0') else '0';

    s_sys_wdata <= s_snoop_data_mux when s_bus_abort='1' else
                   s_cache_out(s_grant_id)(31 downto 0) when s_cache_wb_en(s_grant_id)='1' else (others=>'0');

    -- Ready Logic (Instant, but gated by Abort)
    ...
    gen_cores: for i in 0 to 2 generate
        ...
        u_Cache : entity work.cache port map (
            ...
            o_data_broadcast=>s_brd_data(i), o_mesi_cache=>s_snoop_mesi(i));
